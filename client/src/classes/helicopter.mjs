...
class Helicopter {
  ...
  updateVelocities() {
    ...
    const rollRads =
        this.roll < 0
          ? this.getRadians(150 - this.roll)
          : this.getRadians(150 + this.roll), // Hypothetically 90-this.roll, changed for better playability
      pitchRads =
        this.pitch < 0
          ? this.getRadians(150 - this.pitch)
          : this.getRadians(150 + this.pitch), // Hypothetically 90-this.pitch, changed for better playability
    ...
    if (this.aY > this.gravAOffset) {
      ...
      if (this.roll != 0 && this.pitch != 0) {
        ...
        if (Math.abs(this.roll) > Math.abs(this.pitch)) {
          ...
          this.vX =
            this.roll < 0
              ? -(gravSimY * Math.cos(rollRads))
              : gravSimY * Math.cos(rollRads);
          this.vY = Math.abs(gravSimY * Math.sin(rollRads));
          this.vZ =
            this.pitch < 0
              ? gravSimY * Math.cos(pitchRads)
              : -(gravSimY * Math.cos(pitchRads));
